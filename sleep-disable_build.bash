#!/usr/bin/env bash
# Build Script for sleep-disable
# Information relating to this project is availible from the following URL :
# http://www.lucid.technology/tools/osx/power-management/sleep-disable

# User varibales
package_identifier="com.yourdomain.sleep-disable"
package_version="1.0"
package_output_name="sleep-disable.pkg"

# Intenral variables
exit_status=0
parent_directory="`dirname \"${0}\"`" ; if [ "`echo "${parent_directory}" | grep -e "^/"`" == "" ] ; then parent_directory="`pwd`/${parent_directory}" ; fi
temporary_build_directory=`mktemp -d /tmp/sleep_disable_build_directory.XXXXXXXXXXXXX`
